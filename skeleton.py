...
while True:
    ...
    for id in user.users:
	    ...
	    larm = skel_cap.get_joint_position(id, SKEL_LEFT_HAND)
	    rarm = skel_cap.get_joint_position(id, SKEL_RIGHT_HAND)
	    ...
	    print "  {}: neck at ({loc[0]}, {loc[1]}, {loc[2]}) [{conf}]" .format(id, loc=neck.point, conf=neck.confidence)
            ...
            print "  {}: larm at ({loc[0]},{loc[1]}, {loc[2]}) [{conf}]" .format(id, loc=larm.point, conf=larm.confidence)
            print "  {}: rarm at ({loc[0]},{loc[1]}, {loc[2]}) [{conf}]" .format(id, loc=rarm.point, conf=rarm.confidence)
            print "  {}: waist at ({loc[0]}, {loc[1]}, {loc[2]}) [{conf}]" .format(id, loc=waist.point, conf=waist.confidence)
	    """           
